...
fn start_json_into_options(j: StartJson) -> anyhow::Result<LocalClientOptions> {
    ...
    let (
        ...
        (
            ...
            j.junk_frames.unwrap_or_else(|| 0),
            j.early_ws_frames.unwrap_or_else(|| 0),
            j.decoy_max.unwrap_or_else(|| 0),
            j.max_ws_binary.unwrap_or_else(|| default_max_ws_binary()),
            j.ws_ping_secs.unwrap_or_else(|| default_ws_ping()),
            j.ws_ping_jitter_percent.unwrap_or_else(|| 0),
            j.ws_binary_send_jitter_ms.unwrap_or_else(|| 0),
            j.udp_max_pad,
            j.udp_max_ws_binary,
            j.udp_mux_reply_timeout_secs
