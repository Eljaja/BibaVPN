...
sets TCP_MAXSEG only on the temporary origin listener; the chosen value is logged. This variant inserts an application-data delay proxy; see adjacent README.md.
No public DNS, host routing changes, or privileged containers are used.
"""
import argparse
import json
import os
    ...
    with tempfile.TemporaryDirectory(prefix=tag) as tmp:
        ...
        try:
            ...
            docker('run', '--rm', '-d', '--name', server, '--network', tag,
                   ...
                   '--token', token, '--psk', psk, '--proto-domain', 'bench', '--log-level', 'error', *(['--mux-window-mib', os.environ['BENCH_WINDOW']] if os.environ.get('BENCH_WINDOW') else []))
            origin_port = int(docker('port', origin, '8080/tcp').rsplit(':', 1)[1])
            server_port = int(docker('port', server, '8443/tcp').rsplit(':', 1)[1])
            (lab / 'delay.py').write_text(Path(__file__).with_name('delay-proxy.py').read_text())
            delayed = tag + '-delay'
            owned_containers.append(delayed)
            docker('run', '--rm', '-d', '--name', delayed, '--network', tag,
                   '-p', '127.0.0.1::9443', '-v', f'{lab}:/lab:ro', args.image,
                   'python', '/lab/delay.py', os.environ.get('DELAY_MS','10'), server)
            server_port = int(docker('port', delayed, '9443/tcp').rsplit(':', 1)[1])
            ...
            while True:
                ...
                try:
                    ...
                    time.sleep(.1)
            client_ready_seconds = time.monotonic()-client_started
            ...
            emit(event='setup', label=args.label, client=str(args.client), server=str(args.server),
                 bytes=args.bytes, repeats=args.repeats, client_args=args.client_arg, origin_mss=args.origin_mss, client_ready_seconds=round(client_ready_seconds,4), full_readiness_seconds=round(time.monotonic()-client_started,4), delay_per_direction_ms=float(os.environ.get('DELAY_MS','10')))
