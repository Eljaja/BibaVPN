...
**BibaVPN** is a proxy stack: local **SOCKS5** (TCP **CONNECT** and **UDP ASSOCIATE**) and optional **HTTP CONNECT** → **TLS + WebSocket** → remote entry server → outbound **TCP or UDP** to the Internet. Optionally **BibaV2** (PSK, AEAD, decoy), **Biba v3** (opaque PSK hello/ACK, domain-separated KDF, sealed control frames — backward compatible with v2), and **BibaV2.1** transport knobs (WS ping, frame size cap, custom headers, early noise) sit on top of the transport.
...
- **Requires PSK** on both ends. Client selects v3 with `--proto 3` or invite `proto: 3`.
- **Handshake (NewPath):** first meaningful client Binary after optional noise is either legacy **AUTH** (v2 token path) or `**0x03` + 32 B client random** (opaque hello). Server answers v3 with **32 B server random + 16 B MAC** (no ASCII ACK magic). MAC and session keys use a **domain string** mixed into the KDF (`bibavpn.v3.mac.psk`, `bibavpn.v3.c2s`, `bibavpn.v3.s2c`).
- **Server:** `--proto-domain <label>` (default `default`; must not be empty after trim). **Client / invite:** `--proto-domain` or `proto_domain` in JSON; if the client field is empty, the effective domain is the **SNI** string — it must match what the server uses.
- **Control plane (AUTH, TCP OPEN / OPEN_OK / OPEN_ERR, MUX_OPEN, UDP_MUX_OPEN):** inner **single-byte opcodes** + payloads, carried **inside AEAD** after the handshake — not the plaintext `BIBA\x01…` magics used in v2 for those steps.
- **Data plane:** same padded inner frames and mux/UDP records as v2; **UDP_REQ / UDP_REP** still use the legacy `BIBA…` inner layout inside the ciphertext (handshake is v3; datagram magics not yet migrated).
- `**--print-invite-uri`** today still emits `**proto: 2`** and omits `proto_domain`; for v3 invites use `**bibavpn-mint-invite**` with `INVITE_PROTO=3` and optional `INVITE_PROTO_DOMAIN`, or hand-edit JSON before sealing.
...
- Client `--proto` (`2` default, `3` = v3) and `--proto-domain` (v3 KDF label; empty → SNI)
- Server `--proto-domain` — v3 KDF label (default `default`); must match clients using v3
...
- **WSL v3 + v2 regression** (`wsl-proto-v3-smoke.sh`): release server/client, SOCKS `curl` over v3 then v2.
