#!/usr/bin/env bash
# Lab: bibavpn-server + client (chrome70 + pin-cert), curl via SOCKS, tcpdump on `any`, TLS summary.
# Same flow as wsl-capture-minimal.sh; PCAP name only differs.
# Usage: bash scripts/wsl-capture-lab.sh [PORT] [SOCK_PORT]
# WSL2: use `-i any` — on some installs 127.0.0.1 traffic does not appear on `-i lo`.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
PORT="${1:-18446}"
SOCK="${2:-11091}"
PCAP="/tmp/biba_lab_${PORT}.pcap"
...
CP=$!
sleep 1
echo ok >"/tmp/biba-capture.txt"
python3 -m http.server "${HTTP_PORT}" --directory /tmp >/tmp/biba-capture-http.log 2>&1 &
HPID=$!
sleep 0.5
curl -fsS -o /dev/null --max-time 15 --socks5-hostname "127.0.0.1:${SOCK}" \
  "http://127.0.0.1:${HTTP_PORT}/biba-capture.txt"
sleep 0.3
