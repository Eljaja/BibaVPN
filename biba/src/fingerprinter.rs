...
impl Fingerprinter {
    ...
    pub fn raw_client_hello(&self, raw: &[u8]) -> Result<ClientHelloSpec> {
        let mut spec =
            client_hello_spec_from_tls_record(raw, self.allow_blunt_mimicry, self.real_psk_resumption)?;
...
impl ClientHelloSpec {
    ...
    pub fn always_add_padding(&mut self) {
        let has_pad = self.extensions.iter().any(|e| matches!(e, Extension::Padding { .. }));
        if has_pad {
            return;
        }
