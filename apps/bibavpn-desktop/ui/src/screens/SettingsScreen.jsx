...
export function SettingsScreen({
  ...
  async function handlePickInstalledApp() {
    try {
      const v = await invoke("pick_installed_package_cmd");
      ...
      setCfg((c) => {
        const cur = getActiveProfile(c);
        if (!cur) return c;
        const manual = new Set([...(cur.android_manual_split_packages || [])]);
        manual.add(pkg);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <SettingsSection theme={theme} title={t("settings_group_profile")}>
          <Field
        ...
        <SettingsSection theme={theme} title={t("settings_group_transport")}>
          <CheckRow
            label="use_tcp_mux"
        ...
        <SettingsSection theme={theme} title={t("settings_group_shaping")}>
        ...
        <SettingsSection theme={theme} title={t("settings_group_udp")}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
        ...
        <SettingsSection theme={theme} title={t("settings_group_tls")}>
        ...
        <SettingsSection theme={theme} title={t("settings_group_stealth")}>
          <Field
            label={t("proto_label")}
            type="number"
        ...
        <SettingsSection theme={theme} title={t("settings_group_reality")}>
          <Field
        ...
        <SettingsSection theme={theme} title={t("settings_group_ws_http")}>
        ...
        <SettingsSection theme={theme} title={t("settings_group_split")}>
          {isAndroid && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 10 }}>
              ...
              {p.split_tunnel_enabled && splitGroups.length > 0 && (
                <>
                  ...
                  <Btn kind="solid" block onClick={() => void handlePickInstalledApp()}>
                    {t("android_split_pick")}
                  </Btn>
                  {(p.android_manual_split_packages || []).length > 0 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      <span style={{ fontFamily: "IBM Plex Mono", fontSize: 10, color: theme.textDim }}>
                        {t("android_split_added")}
                      </span>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                        {(p.android_manual_split_packages || []).map((pkg) => (
                          <button
                            key={pkg}
                            type="button"
                            onClick={() => removeManualPkg(pkg)}
                            style={{
                              fontFamily: "IBM Plex Mono",
                              fontSize: 11,
                              padding: "6px 10px",
                  ...
                  <Field
                    label={t("android_manual_packages_label")}
                    value={(p.android_manual_split_packages || []).join("\n")}
                    onChange={(v) => {
                      const manual = v
                        .split(/\r?\n/)
                        .map((x) => x.trim())
                    ...
                    rows={5}
                    hint={t("android_manual_packages_hint")}
                  />
                </>
              )}
            </div>
        ...
        <SettingsSection theme={theme} title={t("settings_group_platform")}>
          <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <span style={{ fontFamily: "IBM Plex Mono", fontSize: 10, color: theme.textDim }}>
              {t("lang_label")}
            </span>
